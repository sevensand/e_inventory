...
handleConfirm(id, e){
  ...
  fetch('/api/approved/' + id, {
    method: 'PUT',
    headers: {
      "Content-Type": "application/json"
    },
    body: JSON.stringify(userrequesitor)
  })
  .then(res => {
      fetch('/api/request')
      .then(res => res.json())
  ...
  handleClickDisapproved(id, e) {
    var userrequesitor = {
      status: this.state.status
    }
    fetch('/api/disapproved/' + id, {
      method: 'PUT',
      headers: {
        "Content-Type": "application/json"
      },
  ...
  search(query){
    const url = `/api/search?status=1&description=${query}`;
    fetch(url)
    ...
    return(
      ...
      <div className="dashcontainer">
              ...
              <TableBody>
              {
                data.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((requesitor, key) => {
                    return(
                        ...
                        <TableCell>
                        <IconButton color="secondary" aria-label="disapproved" onClick={this.handleClickDisapproved.bind(this, requesitor._id)}>
                        <CloseIcon/>
                        </IconButton>
                        <IconButton color="primary"  aria-label="approved" onClick={this.handleClickOpen.bind(this, requesitor._id)}>
                        <DoneIcon />
                        </IconButton>
                        </TableCell>
                        </TableRow>
       ...
       >
         ...
         <Button onClick={this.handleConfirm.bind(this, request._id)} color="primary" autoFocus>
           Confirm
         </Button>
